...
public class RandomSamplers {
	...
	/*Samples number of tables for shared DP 
	 * 	  
	 * @param weights: topic x vertice 
	 * @param maxtable: topic
	 * @return
	 */
	public int[][] randNumTable(double[][] weights, int[] maxtable) {

		//		int customerSum = BasicMath.sum(maxtable);
		int topics = maxtable.length;
		int parents = weights[0].length;

		//		if (topics != weights.length) {
		//			System.out.println("Error, randNumTable expects weights to be topic x vertice.");
		//			return null;
		//		}

		//accumulated weights
		double[][] sumweights = new double[topics][parents];
		for (int i=0;i<topics;i++) {
			sumweights[i][0]=weights[i][0];
			for(int j=1;j<parents;j++) {	
				sumweights[i][j]=weights[i][j]+sumweights[i][j-1];
			}		
		}

		//return number of tables per weight, size |Vertices| * |Topics|
		int[][] ret = new int[parents][topics];

		//create sorted table list for reusing stirling number array for same maxtable values
		int[] index = ArrayTool.sortArray(maxtable);
		...
		for (int i=0;i<topics;i++) {
			...
			//System.out.println(topic + " " + maxtable[i]);

			if (maxtable[i]>0) {

				//if number of customers is different, get new stirling numbers
				if (i == 0  || maxtable[i-1]!=maxtable[i]) {
					if (maxtable[i] > MAXSTIRLING) {
						maxtable[i] = MAXSTIRLING;
					}
					s = stirling(maxtable[i]);
					//System.out.println("got Stirling");
				}

				double sumweight = sumweights[topic][parents-1];

				double[] p = new double[maxtable[i]];
				double pmax = 0;
				//create weights^m * stirling
				for (int table=0;table<maxtable[i];table++) {
					//calculate probability. Note that we use sumweight^(m-1) * stirling(N,m) 
					//dropping terms not containing m and dividing everything by sumweight 
					p[table] = Math.pow(sumweight,table) * s[table];
					//					if (Double.isInfinite(p[table])) {
					//						System.out.println("\nInfinity: " + s[table] + " " + sumweight + " " + table);
					//					}
					//TODO

					//break if results are ==0
					if(p[table] == 0 || (table > 0 && p[table]==p[table-1])) {
						pmax = p[table-1];
						break;
					}
				...
				//sample table
				double randP = Math.random()*pmax;
				//how many tables? Start with 0 (1 table)
				int table=0;

				//System.out.println("first: " + randP + " " + p[table]);

				do {
					//System.out.println(randP + " " + p[table]);

					//sample cluster for adding table
					double rand = Math.random()*sumweight;
					int cluster = 0;
					while (rand > sumweights[topic][cluster]){
						cluster++;
					}
					//add table for sampled cluster
					ret[cluster][topic]++;

					table++;
				} while (randP > p[table-1]);
		...
		//			tablesSum+=BasicMath.sum(ret[i]);
		//		}
		//		if (tablesSum > customerSum) {
		//			
		//			System.out.println("WARNING: tables > customers!");
		//			System.out.println(weights.toString());
		//			System.out.println(maxtable.toString());
		//			
		//		}
		return ret;
	}
	 ...
	 */
	protected int MAXSTIRLING = 1000;
